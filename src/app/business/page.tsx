        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {businesses.map((biz, idx) => (
              <div key={idx} className="flex flex-col bg-white rounded-3xl p-8 border hover:border-amber-400 transition-colors">
                <div className="flex justify-between items-start mb-6">
                  <h3 className="text-2xl font-bold text-[#001F3F]">{biz.title}</h3>
                  <span className={`px-3 py-1 text-sm font-bold rounded-full ${
                    biz.status === "운영중" ? "bg-green-100 text-green-700" :
